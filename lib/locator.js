...
var EXIST_IN_ALL_FUNCTION = internalUtils.read(path.resolve(__dirname, 'browser-scripts', 'existInAll.js'));
...
module.exports.By = function(By) {
    if (!By) {
        throw new Error("Argument not deifned");
    }
...
var each = function (bys) {
    return function (context) {
        ...
        return Q.allSettled(promises).then(function (result_elem_arrays) {
            ...
            return executor.executeScript(EXIST_IN_ALL_FUNCTION, result_elem_arrays.map(function(array) {
                return array.value;
            }));
        });
    };
