...
var ContentUtils = {
    ...
    highlight: function(elems, t) {
        if (Array.isArray(elems)) {
            this.highlightElements(elems);
        } else {
            this.highlightElement(elems, t);
        }
    },
...
ContentUtils.ElementBorder.prototype = {
    ...
    _calcRectForElement : function(elem) {
        if (elem.tagName.toLowerCase() === "map") {
            var imgElement = this._getImageElementForMapElement(elem);
            var rectangle = {
                 left: 0,
                 top: 0,
                 right: 0,
...
// If it exists - it's the time for highlighting. Otherwise it will have a default value of 900 milliseconds.
var t = arguments[2] ? arguments[1] : 900;
ContentUtils.highlight(arguments[0], t);